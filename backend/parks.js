...
const { applySearchArea, fetchPhoto, getPlaceDetails, resolveSearchArea, searchAreaResponse, searchDogParks } = require('./googlePlaces');
const { handleRouteError, normalizeOptionalUrl, requireFields, toOptionalJson } = require('./validation');
const { attachCompatibility } = require('./visitCompatibility');
const { resolveViewerDog } = require('./viewerDog');
const {
  presentVisit,
...
async function searchLocalParks(query, searchArea = null) {
  ...
  const results = parks.map(mapLocalPark);
  return applySearchArea(results, searchArea);
}
...
function visitSelectFields(includeInterested = false, includeViewer = false) {
  return `SELECT v.id, v.user_id AS owner_user_id, v.starts_at, v.duration_minutes, v.status, v.notes, v.social_intent,
              ...
              CASE WHEN d.is_public = 1 THEN d.energy_level ELSE NULL END AS dog_energy_level,
              CASE WHEN d.is_public = 1 THEN d.play_style ELSE NULL END AS dog_play_style,
              CASE WHEN d.is_public = 1 THEN d.social_comfort ELSE NULL END AS dog_social_comfort,
              CASE WHEN d.is_public = 1 THEN d.preferred_dog_sizes ELSE NULL END AS dog_preferred_sizes,
...
function visitGroupFields(includeInterested = false, includeViewer = false) {
  return `GROUP BY v.id, v.user_id, v.starts_at, v.duration_minutes, v.status, v.notes, v.social_intent,
              ...
              d.energy_level, d.play_style, d.social_comfort, d.preferred_dog_sizes${
...
router.get('/search', optionalAuth, async (req, res) => {
  try {
    ...
    if (error.statusCode === 503) {
      let searchArea = null;
      let searchAreaWarning = '';
      try {
        const resolved = await resolveSearchArea(req.query);
        searchArea = resolved.searchArea;
        searchAreaWarning = resolved.warning;
      } catch {
  ...
  try {
    ...
    const devVisits =
      isDevUser
        ...
        : [];
    const viewerDog = await resolveViewerDog(signedInUserId, req.query.viewerDogId);
    const visits = attachCompatibility([...databaseVisits.map((visit) => presentVisit(visit, signedInUserId)), ...devVisits], viewerDog)
    ...
    const devTodayVisits =
      isDevUser
        ...
        : [];
    const todayVisits = attachCompatibility([...databaseTodayVisits.map((visit) => presentVisit(visit, signedInUserId)), ...devTodayVisits], viewerDog)
      .sort((first, second) => String(first.starts_at).localeCompare(String(second.starts_at)))
      .slice(0, 50);
