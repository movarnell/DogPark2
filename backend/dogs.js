...
const { mapDogTraits, normalizeDogTraits, preferredDogSizesJson } = require('./dogTraits');
...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const [dogs] = await db.query(
      `SELECT id, owner_id, name, size, breed, breed_key, avatar_url, energy_level, play_style, social_comfort, preferred_dog_sizes,
              is_friendly, is_puppy, is_public, notes, created_at
       FROM dogs
...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    const traits = normalizeDogTraits(req.body);

    if (isDevAuthUserId(req.user.id)) {
      return res.status(201).json(createDevDog(req.user.id, { ...req.body, ...traits, name: dogName, breed: safeBreed || '', breedKey: safeBreedKey, avatarUrl: safeAvatarUrl }));
    }
    ...
    await db.query(
      `INSERT INTO dogs
       (id, owner_id, name, size, breed, breed_key, avatar_url, energy_level, play_style, social_comfort, preferred_dog_sizes,
        is_friendly, is_puppy, is_public, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        req.user.id,
        ...
        traits.energyLevel,
        traits.playStyle,
        traits.socialComfort,
        preferredDogSizesJson(traits.preferredDogSizes),
...
router.patch('/:id', requireAuth, async (req, res) => {
  try {
    ...
    const traits = normalizeDogTraits(req.body, { partial: true });

    if (isDevAuthUserId(req.user.id)) {
      const dog = updateDevDog(req.user.id, req.params.id, { ...req.body, ...traits, breedKey: safeBreedKey, avatarUrl: safeAvatarUrl });
    ...
    await db.query(
      `UPDATE dogs
       SET name = COALESCE(?, name),
           ...
           play_style = COALESCE(?, play_style),
           social_comfort = COALESCE(?, social_comfort),
           preferred_dog_sizes = COALESCE(?, preferred_dog_sizes),
      ...
      [
        ...
        safeBreedKey,
        safeAvatarUrl,
        traits.energyLevel || null,
        traits.playStyle || null,
        traits.socialComfort || null,
        traits.preferredDogSizes === undefined ? null : preferredDogSizesJson(traits.preferredDogSizes),
