  ...
  try {
    ...
    if (req.query.mine === 'true') {
      ...
      if (isDevAuthUserId(req.user.id)) {
        const viewerDog = await resolveViewerDog(req.user.id, req.query.viewerDogId);
        return res.json(attachCompatibility(listDevVisits({ ownerId: req.user.id, mineOnly: true }), viewerDog));
      }
      where += ' AND v.user_id = ?';
      params.push(req.user.id);
    } else if (req.user && isDevAuthUserId(req.user.id)) {
      const viewerDog = await resolveViewerDog(req.user.id, req.query.viewerDogId);
      return res.json(attachCompatibility(listDevVisits({ ownerId: req.user.id, parkId: req.query.parkId || '' }), viewerDog));
    }
    ...
    const presentedVisits = visits.map((visit) => presentVisit(visit, req.user?.id || ''));
    const viewerDog = await resolveViewerDog(req.user?.id, req.query.viewerDogId);
    res.json(attachCompatibility(presentedVisits, viewerDog));
  } catch (error) {
