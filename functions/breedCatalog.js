...
function breedKey(value) {
  return cleanBreed(value).toLowerCase().replace(/\s+/g, '-');
}

function canonicalBreed(value) {
  const cleaned = cleanBreed(value);
  if (!cleaned) return '';
  const lowered = cleaned.toLowerCase();
  const withoutMix = lowered.replace(/\b(mix|mixed)\b/g, '').replace(/\s+/g, ' ').trim();
  const match = BREED_ALIASES.get(lowered) || BREED_ALIASES.get(withoutMix) || BREEDS.find((breed) => breed.toLowerCase() === lowered || breed.toLowerCase() === withoutMix);
  return match || cleaned;
}

function canonicalBreedKey(value) {
