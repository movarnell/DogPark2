      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-[380px_1fr]">
        <form onSubmit={handleAddDog} className="h-fit rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
          ...
          <label className="mt-4 block text-sm font-bold text-stone-700">
            Dog name
            <input className="mt-1 w-full rounded-md border border-stone-300 px-3 py-2" value={dogName} onChange={(event) => setDogName(event.target.value)} required />
          </label>
          <label className="mt-3 block text-sm font-bold text-stone-700">
            Breed or mix
            <input className="mt-1 w-full rounded-md border border-stone-300 px-3 py-2" value={breed} onChange={(event) => setBreed(event.target.value)} />
          </label>
          <label className="mt-3 block text-sm font-bold text-stone-700">
            Dog size
            <select className="mt-1 w-full rounded-md border border-stone-300 px-3 py-2" value={dogSize} onChange={(event) => setDogSize(event.target.value)}>
          ...
          {dogs.map((dog) => (
            <article className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm" key={dog.id}>
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  className="rounded-md border border-red-200 px-3 py-1 text-sm font-bold text-red-700"
                  type="button"
                  aria-label={`Remove ${dog.name || dog.dog_name}`}
